# Grok 429 Auto Ban Isolated Deployment Design

## Goal

Install `grok-429-autoban` into the production CPA instance without risking the
...
- The candidate Linux plugin is quarantined at
  `plugins-disabled/grok-429-autoban.so`.
...
1. Read and write UTF-8 without transcoding existing bytes through Windows
   PowerShell defaults.
2. Insert only the `grok-429-autoban` configuration block under
   `plugins.configs`.
...
- Config: `canary/grok-429-autoban/config.yaml`
- Plugins: `canary/grok-429-autoban/plugins`
- Auth directory: `canary/grok-429-autoban/auth-dir`
...
2. Logs show `grok-429-autoban` loaded and registered at version `0.1.1`.
...
2. Move `grok-429-autoban.so` back to `plugins-disabled`.
